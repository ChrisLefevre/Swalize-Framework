    ...
    public function plugin_form($document, $structure, $lang)
    {
        ...
        foreach ($swcnt_plugins as $pname) {
            if ($pname == $_GET['plugin']) {
                $doc  = 'ldb/' . $lang . '_plugin_' . $this->format_url($pname) . '_' . $this->format_url($document) . '.source.json';
                $vals = array();
                if (file_exists($doc)) {
        ...
        if (!empty($_GET['plugin']) and !empty($_POST)) {
            foreach ($swcnt_plugins as $pname) {
                if ($pname == $_GET['plugin']) {
                    ...
                    $doc       = 'ldb/' . $lang . '_plugin_' . $this->format_url($pname) . '_' . $this->format_url($document) . '.source.json';
                    $backupdoc = 'ldb/backup/' . $lang . '_plugin_' . $this->format_url($pname) . '_' . $this->format_url($document) . '.' . date("Y-m-d H:i") . 'source.json';
...
class swcnt_sblog extends swcnt_sforms
{
    ...
    function __construct($pubtype = 'blog')
    {
        ...
        /* $pubtype = 'blog','catalog','portfolio' */
        ...
        if (!empty($swcnt_blog['sw_blocks'])) {
            ...
            $doc                 = 'ldb/' . $pubtype . '/' . $actpostId . '.source.json';
            $backupdoc           = 'ldb/backup/' . $pubtype . '/' . $actpostId . '.' . date("Y-m-d H:i") . 'source.json';
            $this->structure     = $structure;
            $this->doc           = $doc;
            $this->backupdoc     = $backupdoc;
            $doctags             = 'ldb/' . $actlang . '_tags_list.source.json';
            $backupdoctags       = 'ldb/backup/' . $actlang . '_tags_list.' . date("Y-m-d H:i") . 'source.json';
...
class swcnt_sblog_cat extends swcnt_sforms
{
    ...
    function __construct($pubtype = 'blog')
    {
        ...
        if (!empty($swcnt_blogcatform)) {
            ...
            $doc             = 'ldb/' . $pubtype . '/' . $actlang . '_cats.source.json';
            $backupdoc       = 'ldb/backup/' . $pubtype . '/' . $actlang . '_cats' . date("Y-m-d H:i") . 'source.json';
            $this->structure = $structure;
            $this->doc       = $doc;
            $this->backupdoc = $backupdoc;
        }
...
class swcnt_smod extends swcnt_sforms
{
    ...
    public function listplugins()
    {
        if (!empty($_GET['plugin']))
            $smod->plugact = $_GET['plugin'];
        else
            $smod->plugact = '';
        if (!empty($_GET['plugpage']))
            $smod->plugpage = $_GET['plugpage'];
        else
            $smod->plugpage = '';
...
?>
                ...
                foreach ($v['pages'] as $tpage => $vpage) {
                    $a_ = ($smod->plugpage == $tpage) ? 'class="active"' : '';
