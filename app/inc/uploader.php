...
&& in_array($extension, $allowedExts))
  ...
  if ($_FILES["file"]["error"] > 0)
  ...
  else
    {
      ...
      "upload/temp/" . date("Ymd-His.").$extension);
     
	  $filename = "upload/temp/" . date("Ymd-His.").$extension;
	  $addimage = "upload/gallery/" . date("Ymd-His").".jpg";
	  $thumbimage = "upload/thumb/" . date("Ymd-His").".jpg";
	  $tmpnimg = date("Ymd-His").".jpg";
...
$new_width = $squaresize;
$percent = $squaresize/$width;
$new_height = round($height * $percent);
} else {	
$new_height = $squaresize;
$percent = $height/$squaresize;
$new_width = round($width / $percent);
}



$image_t = imagecreatetruecolor($squaresize, $squaresize);
if(findexts($filename)=='jpg' or findexts($filename)=='jpeg')  $image = imagecreatefromjpeg($filename);
else if(findexts($filename)=='gif')  $image = imagecreatefromgif($filename);
else if(findexts($filename)=='png')  $image = imagecreatefrompng($filename);

imagecopyresampled($image_t, $image, 0, 0, $new_width-$squaresize, $new_height-$squaresize, $new_width, $new_height, $width, $height);
